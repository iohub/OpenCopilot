...
export class ClineProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "ocopilot-dev.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "ocopilot-dev.TabPanelProvider"
	...
	*/
	...
	async switchToChatView() {
		if (this.view && this.inClineView) {
			...
			await this.sendClineMessagesToTranscript()
		}
	}
				...
				switch (message.type) {
					...
					case "openFile":
						...
						if (message.text === "{SystemPromptFile}") {
							file = path.join(os.homedir(), ".ocopilot", AlineGlobalFileNames.systemPrompts)
						}
						if (message.text === "{ModelProviderFile}") {
							file = path.join(os.homedir(), ".ocopilot", AlineGlobalFileNames.modelOptions)
						}
						openFile(file!)
						break
					case "openMention":
					...
					case "loadModelProvider": {
						const options = await this.loadModelProvider()
					...
					case "switchWebview": {
						if (message.text === 'Chat') {
							await this.switchToChatView()
						} else if (message.text === 'CLINE') {
							await this.switchToClineView()
						}
						break
					}
					case "updateProviderConfig": {
						if (message.providerConfig) {
							await this.updateGlobalState(`providerCfg-${message.providerConfig.provider}`, message.providerConfig)
							await this.postStateToWebview()
						}
						break
					}
	...
	*/
	...
	async getState() {
		const [
			...
			this.getGlobalState("modelOptions") as Promise<ModelProvider[] | undefined>,
		])
		...
		const providerCfg = await this.getGlobalState(`providerCfg-${apiProvider}`) as ProviderConfig
		return {
			apiConfiguration: {
				apiProvider,
	...
	async getGlobalState(key: string) {
	...
	private async loadModelProvider() : Promise<ModelProvider[]> {
		...
		const filePath = path.join(dirPath, AlineGlobalFileNames.modelOptions)
		if (!(await this.fileExists(filePath))) {
			await fs.writeFile(filePath, DEFAULT_MODEL_PROVIDER_STR, "utf8")
		}
		return await JSON.parse(await fs.readFile(filePath, "utf8"))
	}
	...
	public async sendClineMessagesToTranscript() {
		...
		const startIndex = this.cline.clineMessages.findIndex(msg => 
			msg.type === "say" && msg.say === "api_req_started"
		)
		if (startIndex === -1) { return }
		this.cline.clineMessages.slice(startIndex).forEach(msg => {
			if (msg.type === "say" && (msg.say === "text" || msg.say === "api_req_started")) {
				transcripts.push(toChatMessage(msg))
			}
		})
		await this.sendTranscripts(this.view, transcripts, false, this.cline?.taskId || "chat-default")
	}
	
	 /**
     * Send transcript to webview
     */
	 async sendTranscripts(view:  vscode.WebviewView | vscode.WebviewPanel, 
		transcripts: ChatMessage[], inProgress: boolean, chatID: string) {
