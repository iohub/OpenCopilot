...
import { removeMiddleImageBlockParam } from "../utils/AlineUtil"
...
export class Cline {
	...
	inClineBuiltinMode() : boolean {
		return this.systemPrompt.id.toLocaleLowerCase() === "cline"
	}
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		let systemPrompt = this.inClineBuiltinMode() ?
			await SYSTEM_PROMPT(cwd, this.api.getModel().info.supportsComputerUse ?? false, mcpHub, !mcpHub.isUnconnected()) :
		...
		const stream = this.api.createMessage(systemPrompt, removeMiddleImageBlockParam(this.apiConversationHistory))
