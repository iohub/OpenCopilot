...
export class BotResponseMultiplexer {
    ...
    public async notifyTurnComplete(): Promise<void> {
        ...
        await Promise.all(Array.from(this.subs_.values()).map(subscriber => subscriber.onTurnComplete()))
    }
    ...
    public prompt(): string {
        return `Enclose each part of the response in one of the relevant tags: ${Array.from(this.subs_.keys())
            .map(topic => `<${topic}>`)
            .join(', ')}:\n\n`
    }
}
